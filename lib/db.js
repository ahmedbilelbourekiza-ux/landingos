...
const stmts = {
  ...
  insertNotification: db.prepare(`INSERT INTO notifications
    (type, title, body, orderId, shipmentId) VALUES (@type, @title, @body, @orderId, @shipmentId)`),
...
function loadOrdersData() {
  return {
    ...
    agents: stmts.allAgents.all().map(a => ({ name: a.name, pass: a.pass, role: a.role || 'confirmation', missedOrders: a.missedOrders || 0, suspended: !!a.suspended, weeklyDaysOff: safeParseDaysOff(a.weeklyDaysOff) })),
  };
}
...
function getAgents() { return stmts.allAgents.all().map(a => ({ name: a.name, pass: a.pass, role: a.role || 'confirmation', missedOrders: a.missedOrders || 0, suspended: !!a.suspended, baseSalaryMonthly: a.baseSalaryMonthly || 0, payPerConfirmedOrder: a.payPerConfirmedOrder || 0, payPerDeliveredOrder: a.payPerDeliveredOrder || 0, weeklyDaysOff: safeParseDaysOff(a.weeklyDaysOff) })); }
...
function addAgent(name, pass, role) {
  db.prepare('INSERT INTO agents(name,pass,role) VALUES(?,?,?)').run(name, pass || '', role || 'confirmation');
}
...
function markNotificationsRead(olderThanId = null) {
  if (olderThanId) db.prepare('UPDATE notifications SET read = 1 WHERE id <= ?').run(olderThanId);
