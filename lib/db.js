...
/* =============================================================================
 * INDEXES (audit PERF-01).
 *
 * The busiest table in the system had exactly ONE index (phoneNormalized,
 * above). Everything else — every status filter, every per-agent lookup, the
 * ORDER BY on the main list, the webhook dedup by external id — was a full
 * table scan.
 *
 * Worse, `order_calls` had none at all. SQLite does NOT index a FOREIGN KEY
 * automatically, so attachCalls() scanned the entire calls table once PER
 * ORDER, making GET /api/orders quadratic. Measured on 5,000 orders before
 * this change: loadOrdersData() took 3,006 ms, and the frontend calls it on
 * every SSE event and again every 30 seconds.
 *
 * These are pure additions — no column or row changes — so they are safe to
 * create on an existing database. SQLite builds them on first boot after the
 * upgrade; on a large table that costs a few seconds once.
 * ========================================================================== */
/* Created one at a time rather than in a single db.exec() block. An index over
 * a column that does not exist raises, and inside one statement batch that
 * takes the whole process down at boot — turning a missing optimisation into a
 * total outage. Individually, a failure is logged and the server still starts;
 * queries just fall back to the scans they used before. */
const PERFORMANCE_INDEXES = [
  // The main list: ORDER BY createdAt DESC was a scan plus a temp B-tree sort.
  'CREATE INDEX IF NOT EXISTS idx_orders_created        ON orders(createdAt DESC)',
  // Status filters, the dashboard buckets, and the stale-order sweep.
  'CREATE INDEX IF NOT EXISTS idx_orders_status_created ON orders(status, createdAt DESC)',
  // "My orders" for an agent, and the auto-assign workload count.
  'CREATE INDEX IF NOT EXISTS idx_orders_agent_status   ON orders(agent, status)',
  'CREATE INDEX IF NOT EXISTS idx_orders_followup_agent ON orders(followupAgent)',
  // Webhook dedup: every inbound order looks itself up by the platform's id.
  'CREATE INDEX IF NOT EXISTS idx_orders_shopify_id     ON orders(shopifyId)',
  'CREATE INDEX IF NOT EXISTS idx_orders_store          ON orders(storeId, shopifyId)',
  // Delivery webhooks arrive keyed by tracking number.
  'CREATE INDEX IF NOT EXISTS idx_orders_tracking       ON orders(trackingNumber)',
  // The profit calculator and delivered-pay payroll both filter on this pair.
  'CREATE INDEX IF NOT EXISTS idx_orders_outcome        ON orders(deliveryOutcome, deliveryOutcomeAt)',
  // The quadratic one: one scan of the whole table per order, via attachCalls().
  'CREATE INDEX IF NOT EXISTS idx_order_calls_order     ON order_calls(orderId, id)',
  // Shipment lookups by order and by tracking number are on the webhook path.
  ...
  'CREATE INDEX IF NOT EXISTS idx_shipments_provider    ON shipments(providerId)',
];
for (const sql of PERFORMANCE_INDEXES) {
  try { db.exec(sql); }
  catch (e) { console.error('[db] could not create an index (queries will fall back to a scan): ' + e.message); }
}

/* ANALYZE lets SQLite's planner choose between the new indexes using real
 * row-distribution statistics rather than guesswork. Cheap, and only run when
 * the statistics table is missing or empty — i.e. once. */
try {
  const hasStats = db.prepare("SELECT COUNT(*) AS c FROM sqlite_master WHERE type='table' AND name='sqlite_stat1'").get().c;
  if (!hasStats) db.exec('ANALYZE');
} catch (e) { /* statistics are an optimisation, never a requirement */ }
...
/* =============================================================================
 * queryOrders — filtered, sorted, paginated orders in SQL (audit PERF-02).
 *
 * loadOrdersData() returns EVERY order with EVERY call attached: 291 ms on
 * 5,000 orders even after the indexes, and the console re-runs it on every SSE
 * event and again every 30 seconds. All the filtering the UI does — status,
 * agent, date range, search — was happening in the browser over that full
 * download.
 *
 * Call history is deliberately NOT attached here. It is only needed when a
 * single order is opened, and joining it for a list page is what made the old
 * path quadratic in the first place. `callCount` is included instead, since
 * that is all the list actually renders.
 *
 * Every value is bound, never interpolated; `sort` is whitelisted.
 * ========================================================================== */
const ORDER_SORT_COLUMNS = {
  createdAt: 'createdAt', price: 'price', client: 'client',
  status: 'status', agent: 'agent', updatedAt: 'updatedAt',
};
...
function buildOrderFilter(f = {}) {
  ...
  // Record-level scope. Applied in SQL rather than after the fact, so a
  // paginated page of 50 is 50 rows this caller may actually see — filtering
  // post-query would silently return short pages.
  if (f.scopeAgent) {
    where.push("(agent = ? OR followupAgent = ? OR agent IS NULL OR agent = '')");
    params.push(f.scopeAgent, f.scopeAgent);
  }
  if (f.status)         { where.push('status = ?');           params.push(f.status); }
  if (f.agent)          { where.push('agent = ?');            params.push(f.agent); }
  if (f.followupAgent)  { where.push('followupAgent = ?');    params.push(f.followupAgent); }
  if (f.wilaya)         { where.push('wilaya = ?');           params.push(f.wilaya); }
  if (f.storeId)        { where.push('storeId = ?');          params.push(f.storeId); }
  if (f.orderType)      { where.push("COALESCE(orderType,'normal') = ?"); params.push(f.orderType); }
  if (f.classification) { where.push("COALESCE(classification,'') = ?");  params.push(f.classification); }
  if (f.deliveryOutcome){ where.push("COALESCE(deliveryOutcome,'') = ?"); params.push(f.deliveryOutcome); }
  if (f.since)          { where.push('createdAt >= ?');       params.push(Number(f.since)); }
  if (f.until)          { where.push('createdAt <= ?');       params.push(Number(f.until)); }

  if (f.search) {
    const q = '%' + String(f.search).trim().toLowerCase() + '%';
    where.push('(LOWER(client) LIKE ? OR phone LIKE ? OR LOWER(id) LIKE ? OR LOWER(product) LIKE ? OR LOWER(COALESCE(shopifyName,\'\')) LIKE ? OR COALESCE(trackingNumber,\'\') LIKE ?)');
...
function queryOrders(filters = {}) {
  const { clause, params } = buildOrderFilter(filters);
  const sortCol = ORDER_SORT_COLUMNS[filters.sort] || 'createdAt';
  const dir = String(filters.dir).toLowerCase() === 'asc' ? 'ASC' : 'DESC';
  const limit = Math.min(Math.max(Number(filters.limit) || 50, 1), 200);
  const offset = Math.max(Number(filters.offset) || 0, 0);

  const total = db.prepare(`SELECT COUNT(*) AS c FROM orders ${clause}`).get(...params).c;
  const rows = db.prepare(
    `SELECT o.*, (SELECT COUNT(*) FROM order_calls c WHERE c.orderId = o.id) AS callCount
     FROM orders o ${clause} ORDER BY ${sortCol} ${dir}, o.id ${dir} LIMIT ? OFFSET ?`
  ).all(...params, limit, offset);

  return {
    items: rows.map(r => ({ ...rowToOrder(r), callCount: r.callCount || 0 })),
    total, limit, offset,
    hasMore: offset + rows.length < total,
  };
}

/** Aggregate counts for the whole filtered set, so the dashboard tiles do not
 *  need the rows themselves. */
function countOrdersByStatus(filters = {}) {
  const { clause, params } = buildOrderFilter(filters);
  const rows = db.prepare(`SELECT status, COUNT(*) AS c FROM orders ${clause} GROUP BY status`).all(...params);
  const out = { total: 0 };
  for (const r of rows) { out[r.status || 'unknown'] = r.c; out.total += r.c; }
  return out;
}
...
/* The fields whose transitions the client and product lifetime counters key
 * off. If a write touches none of them, those upserts cannot possibly change
 * anything and are skipped — see saveOrder (audit PERF-03). */
const STAT_RELEVANT_FIELDS = ['status', 'deliveryOutcome', 'price', 'phone', 'product', 'quantity', 'shopifyProductId'];

function saveOrder(o, opts = {}) {
  /* `before` is passed in by patchOrder, which has just read the row anyway.
   * This path used to call getOrder() FIVE times for a single field change —
   * once for the patch merge, once for `before`, twice to feed the two stat
   * upserts, and once to return — each of them re-reading the row and its call
   * history. */
  const before = opts.before !== undefined ? opts.before : getOrder(o.id);
  const row = orderToRow(o);

  // Only recompute lifetime stats when something they actually depend on moved.
  const statsMayHaveChanged = !before || STAT_RELEVANT_FIELDS.some(f => before[f] !== o[f]);
  ...
  const tx = db.transaction(() => {
    ...
    if (statsMayHaveChanged) {
      ...
      upsertClientFromOrder(before, after);
      upsertProductStatsFromOrder(before, after);
    }
  });
  tx();
  return getOrder(o.id);
...
function patchOrder(id, patch) {
  ...
  if (!cur) return null;
  // Hand `cur` down as the before-state instead of making saveOrder re-read it.
  return saveOrder({ ...cur, ...patch, id }, { before: cur });
}
...
module.exports = {
  ...
  loadOrdersData, queryOrders, countOrdersByStatus,
  getOrder, findOrder, insertOrder, saveOrder, patchOrder, deleteOrder, addCall,
