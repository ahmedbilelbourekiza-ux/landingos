...
function ingestTrackingEvents(shipment, events, broadcaster, opts = {}) {
  ...
  if (shouldNotify(latest.crmStatus)) {
    ...
    /* Via notifications.push(), not db.addNotification(): the direct call
     * stored a row with no audience and never broadcast it, so a delivery
     * update was invisible until someone reloaded — and then visible to
     * everyone. Addressed to whoever owns the parcel (the follow-up agent if
     * one is assigned, else the confirming agent); managers see it either way. */
    require('../notifications').push({
      type: 'delivery_' + latest.crmStatus,
      title, body,
      orderId: order?.id || '', shipmentId: updated.id,
      target: order?.followupAgent || order?.agent || null,
    });
  }
