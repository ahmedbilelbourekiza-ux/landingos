  ...
  const alertOnce = () => {
    try {
      ...
      if (stale.length > 0) {
         ...
         * Routed through notifications.push() rather than db.addNotification()
         * directly, so the row carries an audience and the live event carries
         * the id needed for replay. MANAGER_ONLY: "N orders nobody has called"
         * is a supervision signal, not something to push at the agents. */
        notifications.push({
          type: 'stale_orders',
  ...
  const followupOnce = () => {
    try {
      const escalated = followup.sweepOverdue((n) => {
        // An escalation means an agent let a reminder expire — a manager
        // signal. push() stores and broadcasts in one step.
        notifications.push({
