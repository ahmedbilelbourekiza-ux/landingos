...
function autoAssign() {
  try {
    ...
    // orders are eligible — a suspended agent must never receive new work,
    // and a followup-only agent isn't part of this pipeline.
    const agents = (data.agents || []).filter(a => !a.suspended && (a.role === 'confirmation' || a.role === 'both' || !a.role));
    if (!s.autoAssign) {
  ...
  for (const order of overdue) {
    ...
    if (s.autoReassign) {
      try {
        ...
        const candidates = confirmationAgents.filter(a => a.name !== originalAgent && !a.suspended);
    ...
    try {
      ...
      if (order) {
        ...
        const newOrder = {
          ...
          shippingNote:   '',
          managerNote:    '',
          shopifyId:      order.shopifyId ? String(order.shopifyId) : '',
...
app.get('/api/clients', (req, res) => {
  ...
  const sort = req.query.sort || 'lastOrderAt';
  const limit = Math.min(Number(req.query.limit) || 50, 200);
  const offset = Number(req.query.offset) || 0;
  res.json({
    items: db.getClients({ search, sort, limit, offset }),
    total: db.countClients(search),
  });
});
