...
| **LB.13** | Editor i18n (the legacy editor speaks English in an ar/fr console) | M | **IN PROGRESS 11 Aug 2026 — `EDITOR_I18N.md` is the slice log.** Commercial for the Arabic-first market. The storefront half (the thank-you page) closed in LB.10. The measurement corrects M-04 twice: the create screen is ALREADY translated, and 10 of the "54 components" are dead legacy-dashboard code reachable from nothing (removal candidates, `EDITOR_I18N.md` §3). Real scope: 48 live components, 166+ strings. LB.13a (shell + section frame + the save path's error text) is done |
