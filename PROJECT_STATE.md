...
**Last updated:** 16 August 2026 (later session) — everything through
**LB.50** is **DEPLOYED**; **LB.51–LB.53 (the TTFB query-census fix, the
gallery-warming trade, the price-contrast fix) are COMMITTED on branch
`claude/perf-ttfb-images-2otrah` and NOT deployed** — five commits on top
of `831c48d`, no migration, live before/after verification owed (that
session's environment could not reach production; NEXT_STEPS §LB.51 has
the exact steps). **The first real custom domain (`selliora1.com`) serves
