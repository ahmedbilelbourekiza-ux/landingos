...
**Branch:** `master`, in sync with `origin/main` at `964755b` · **Everything
through LB.39 is deployed:** LB.31–LB.36 + LB.15 + LB.14a/b/c as `d6a56b1`,
**LB.37** (`fcbd1e5`) the storefront `<head>` fix, then **LB.38** (`a70f588`)
the Delete door and **LB.39** (`dbe1cf0`) the per-tenant sitemap. **Nothing is
queued — `git diff origin/main master -- apps packages` is the check, and no
migration is pending.** **Still unbuilt and worth knowing: LB.35's per-page
pixel control has no UI at all** (§LB.35b) and `robots.txt` is a deliberate
open decision (§LB.39). LB.35's column was applied to
