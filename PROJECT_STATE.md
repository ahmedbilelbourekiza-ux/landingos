...
verified. **Same session, evening: LB.54 (the digit-only-slug fix — the
real `selliora1.com/0` 404 traced to `slugify` reducing an Arabic title
to its digits) is BUILT on local branch `claude/fix-digit-only-slugs`,
suite-covered (builder-api 42→49), NOT deployed and NOT pushed per the
user's instruction; LB.24's first slice sits likewise unpushed on
`claude/lb24-ai-generator`.** **The first real custom domain
