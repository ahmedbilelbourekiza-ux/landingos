...
**Last updated:** 14 August 2026 — everything through **LB.40**
is **DEPLOYED**
**Branch:** `master`, in sync with `origin/main` at `0286f99` · **Everything
through LB.40 is deployed:** LB.31–LB.36 + LB.15 + LB.14a/b/c as `d6a56b1`,
**LB.37** (`fcbd1e5`) the storefront `<head>` fix, **LB.38** (`a70f588`)
the Delete door, **LB.39** (`dbe1cf0`) the per-tenant sitemap, **LB.35b**
(`dd4edac`) the per-page pixel control, and **LB.40** (`f1e38bf`) `robots.txt`. **Nothing is
