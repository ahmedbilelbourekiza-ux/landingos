...
**Last updated:** 13 August 2026, late night — everything through **LB.39**
and **LB.35b** is **DEPLOYED**
**Branch:** `master`, in sync with `origin/main` at `407854a` · **Everything
through LB.39 plus LB.35b is deployed:** LB.31–LB.36 + LB.15 + LB.14a/b/c as `d6a56b1`,
**LB.37** (`fcbd1e5`) the storefront `<head>` fix, then **LB.38** (`a70f588`)
the Delete door, **LB.39** (`dbe1cf0`) the per-tenant sitemap, and **LB.35b** (`dd4edac`) the per-page pixel control. **Nothing is
queued — `git diff origin/main master -- apps packages` is the check, and no
migration is pending.** **LB.35's missing per-page pixel control
