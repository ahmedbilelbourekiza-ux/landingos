...
| **N4** | **Notification sounds** | six distinct Web Audio signatures — `new_order` (ka-ching), `abandoned`, `assignment`, `manipulation` (siren), `delivery`, `followup` — with a per-type toggle and a volume, persisted in `localStorage` | ✅ **CLOSED — LP.11**: all six ported note for note, with a test button beside each toggle and a CLAMPED volume. Persisted on `ProductSetting` rather than `localStorage`, so it follows the person between devices — and so a supervisor can tell whether an agent has silenced the siren that watches them. |
| **N5** | **Desktop notifications** | `Notification` API on every event, tagged by order id so they collapse | ✅ **CLOSED — LP.11**, with two corrections to the legacy: it fires only when the tab is NOT visible (otherwise it duplicates the toast beside it), and permission is requested on a CLICK rather than on page load — asking on load trains people to click Block, and a blocked permission can never be re-requested by that origin. Still tagged by entity, so six carrier events for one parcel replace each other. |
...
| **N9** | **Inline row actions** | board card carries agent select, carrier select, express toggle and status select — three changes without leaving the list | click into the detail page, once per order | ✅ **CLOSED — LP.8**: all four on the row, each calling `PATCH /orders/[id]`, each gated by the predicate that route uses (`AGENT_WRITABLE` vs REASSIGNMENT vs `MANAGER_WRITABLE`) — so an agent gets two and a manager four. |
| **N10** | **List information density** | per row: order type badge (draft/abandoned/normal), note badge with the note as a tooltip, fake badge, overdue tag with a pulsing dot, store logo + name + brand + platform, date **and** time, variant thumbnail, and qty / unit / delivery / discount / total | reference, customer, destination, product, total, status, call count, date — roughly a third of the information | ✅ **CLOSED — LP.8**: type · fake · overdue · flagged-call · note (the note IS the tooltip) · channel + brand + platform · date and time · variant and quantity · delivery status and tracking · the money broken down. The product IMAGE is deliberately not restored: ERP images are base64 in the database (M-14) and fifty of them on one page is the payload PERF-02 exists to prevent. |
| **N11** | **Payroll report** | modal from the agents screen | `/api/erp/agents/payroll` exists; **nothing renders it** | ✅ **CLOSED — LP.12**: rendered on the roster for the aligned current month, under the proration rule LP.16b made shared. |
| **N12** | **Audit-log view** | investigation modal per order | `/api/erp/orders/[id]/audit` exists; the detail screen does not render it | ✅ **CLOSED — LP.12**, and it was hiding a defect: `AuditEvent`, the route and the permission all existed while **no order mutation wrote a row**. `PATCH /orders/[id]` records the edit now — which fields, never their values. |
...
| **N21** | **Order-row density, measured** — ✅ **CLOSED — LP.8**, and the four that decide are the four restored first | 14 facts: order number · **type badge** (draft/abandoned/normal) · **note badge with the note as its tooltip** · **fake badge** · **overdue tag with a pulsing dot** · store logo + name + brand + platform · date **and** time · customer · phone · wilaya + commune · **variant thumbnail** · qty · unit price · delivery cost · discount · total · status · agent · carrier | 8: reference · customer + phone · destination · product · total · status · call count · date. **The four that decide what to do next are all missing** — overdue, called, noted, flagged. The call count is the one equivalent and answers a weaker question: three failed calls and three callbacks look identical. | 🟡 |
| **N22** | **The changed row flashes** | `hl-flash`, a 3s animation on the row or card that just moved — on the list and the board | ✅ **CLOSED — LP.8**: `components/console/row-flash.ts` plus one keyframe. It MARKS and never merges. It retries until the row exists (bounded ≈2.4 s) so it survives LP.7's 500 ms debounced refresh re-rendering the table underneath it, and `prefers-reduced-motion` keeps the highlight while dropping the fade. | ✅ |
...
### Tier 2 — daily operator productivity — **COMPLETE**
