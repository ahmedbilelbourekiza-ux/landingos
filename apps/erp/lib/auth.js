...
/* A single scrypt derivation at these parameters costs ~100-200ms of CPU.
 * scryptSync would spend all of that BLOCKING the event loop, so a handful of
 * concurrent logins — or one attacker with a loop — would stall every other
 * request on the server. The async form runs on the libuv threadpool instead,
 * so the process keeps serving while a password is being checked. */
...
/* Nothing legitimate is longer than this, and every extra byte is work an
 * unauthenticated caller can make the server do. The JSON body limit is 25MB
 * (needed for base64 product images), so without a cap here a login request
 * could hand scrypt a 25MB "password". */
...
async function hashPassword(plain) {
  ...
  const key = await scrypt(normalizePassword(plain), salt, SCRYPT.keylen, {
...
async function verifyPassword(plain, stored) {
  ...
  let derived;
  try {
    derived = await scrypt(candidate, Buffer.from(saltHex, 'hex'), keyHex.length / 2, {
...
/* A fixed hash to check a submitted password against when the account does not
 * exist, so a miss costs the same one derivation a hit does. Building a fresh
 * decoy per request instead cost TWO derivations (hash + verify), which made
 * unknown accounts measurably slower than real ones — leaking exactly what the
 * uniform error message was meant to hide, only backwards. Generated once,
 * lazily, and never used to authenticate anything. */
let DECOY_HASH = null;
async function verifyAgainstDecoy(plain) {
  if (!DECOY_HASH) DECOY_HASH = await hashPassword(crypto.randomBytes(32).toString('hex'));
  return verifyPassword(plain, DECOY_HASH);
}
...
function resolveSession(rawToken) {
  ...
  /* lastSeenAt was written on EVERY authenticated request — an UPDATE per API
   * call, against a database with a single writer. Under any real load that is
   * pure write contention for a field nobody reads more precisely than
   * "roughly when". Once every few minutes is plenty. */
  if (!row.lastSeenAt || (Date.now() - row.lastSeenAt) > SESSION_TOUCH_INTERVAL_MS) {
 ...
 */
async function migratePlaintextPasswords(log) {
  const rows = db.listAgentSecrets();
  let migrated = 0, blank = 0;
  for (const a of rows) {
    if (isHashed(a.pass)) continue;
    const hasPassword = !!a.pass;
    // The third argument was previously omitted, so it defaulted to `true` and
    // every migrated account — INCLUDING the blank-password ones this flag
    // exists to expose — reported hasPassword: true. The warning below counted
    // them correctly while the UI showed them as fine.
    db.setAgentPasswordHash(a.name, await hashPassword(a.pass ?? ''), hasPassword);
    migrated++;
    if (!hasPassword) blank++;
  }
  if (migrated && log) {
 ...
 */
async function ensureManagerAccount(log) {
  ...
  if (existing) {
    ...
    db.setAgentPasswordHash(username, await hashPassword(password), true);
  ...
  db.addAgent(username, await hashPassword(password), 'both', { accountRole: 'manager', hasPassword: true });
...
module.exports = {
  ...
  hashPassword, verifyPassword, verifyAgainstDecoy, isHashed, MAX_PASSWORD_BYTES,
