...
/* Maps the French labels this adapter actually emits back to CRM statuses.
 * Previously this was `{}` with a comment claiming the mock "already speaks CRM
 * statuses directly" — but createShipment() returns PIPELINE[0].label
 * ("Création"), not the CRM key, so mapStatus() fell through to the shared
 * keyword fallback and resolved a brand-new shipment to 'pending' instead of
 * 'created'. Deriving the map from PIPELINE keeps the two in step forever. */
const statusMap = Object.fromEntries(PIPELINE.map(s => [s.label, s.crm]));
